...
type AlgoProfileOrigRepo interface {
	...
	GetWinRatioProfile(ctx context.Context, userId string, userType int32, algoId int32, start, end int64) ([]*models.TbAlgoProfileOrig, error)

	//reload
	ReloadProfiles(ctx context.Context, date int64) ([]*models.TbAlgoProfileOrig, error)

	// exception process
	GetDataByProfileKey(ctx context.Context, date int64, userId string, algoId int, secId string, algoOrderId int64) (models.TbAlgoProfileOrig, error)
}

type DefaultAlgoProfileOrig struct {
	DB *gorm.DB
}

func NewAlgoProfileOrig(conn *gorm.DB) AlgoProfileOrigRepo {
	return &DefaultAlgoProfileOrig{
		DB: conn,
	}
}

func (d *DefaultAlgoProfileOrig) CreateAlgoProfile(ctx context.Context, profile *global.Profile) error {
	p := models.TbAlgoProfileOrig{
...
func (d *DefaultAlgoProfileOrig) UpdateAlgoProfile(ctx context.Context, profile *global.Profile) error {
	result := d.DB.Model(models.TbAlgoProfileOrig{}).
		Where("date=? and batch_no=? and account_id=? and algo_id=? and sec_id=? and algo_order_id=?",
			profile.Date, profile.BatchNo, profile.AccountId, profile.AlgoId, profile.SecId, profile.AlgoOrderId).
...
// GetProfiles 明细返回
func (d *DefaultAlgoProfileOrig) GetProfiles(ctx context.Context, in *proto.AlgoProfileReq, algoType int32) ([]*models.TbAlgoProfile, int64, error) {
	...
	var count int64
	// 用魔法打败魔法
	result := d.DB.Model(&models.TbAlgoProfileOrig{}).Select("batch_no, account_id, account_name, account_type,provider,algo_type, algo_id,algo_name," +
	...
	//}
	if a := in.BatchNo; a != 0 && a != 1 {
		result.Where("batch_no=?", a)
	}
...
func (d *DefaultAlgoProfileOrig) GetWinRatioProfile(ctx context.Context, userId string, userType int32, algoId int32,
	start, end int64) ([]*models.TbAlgoProfileOrig, error) {
	var out []*models.TbAlgoProfileOrig
	result := d.DB.Model(&models.TbAlgoProfileOrig{}).Select("date,algo_name, avg(profit_rate) as profit_rate,sum(profit_amount) as profit_amount," +
...
func (d *DefaultAlgoProfileOrig) ReloadProfiles(ctx context.Context, date int64) ([]*models.TbAlgoProfileOrig, error) {
	var out []*models.TbAlgoProfileOrig
	result := d.DB.Model(&models.TbAlgoProfileOrig{}).Select("date,account_id,algo_id,trade_cost,total_trade_fee,cross_fee,profit_amount,profit_rate,"+
...
func (d *DefaultAlgoProfileOrig) GetDataByProfileKey(ctx context.Context, date int64,
	userId string, algoId int, secId string, algoOrderId int64) (models.TbAlgoProfileOrig, error) {
	var out models.TbAlgoProfileOrig
	result := d.DB.Model(models.TbAlgoProfileOrig{}).Select("account_id").
	...
	if result.Error != nil {
		return models.TbAlgoProfileOrig{}, result.Error
	}
	return out, nil
}
